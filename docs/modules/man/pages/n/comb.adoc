...
*[cmd]#comb#*  [[-c] | [-r]] [[-w] | [-f] | [-l]] [[-S]] {[rep]_combination_name_} [[rep]_<operator object_name>_]...
...
====
 When a combination contains only unioned objects, it can be flattened with the *[opt]#f#*  option. ::
...
====
 The *[opt]#l#*  option is useful in cases where many *[opt]#w#*  type operations are needed. ::
...
====
 While the *[opt]#l#*  option can handle many tree configurations, there are some situations where manual intervention is required.  This example illustrates one such case. ::
Suppose a geometry has the following structure:
