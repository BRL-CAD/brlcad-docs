...
|===
2+| Usage: gdiff [OPTION]... obj1 obj2
...
|[nowrap]`--sections=[replaceable]#list#`
...
|[nowrap]`--bot-plate-thickness=[replaceable]#thickness#`
...
|Creates a plot/overlay (`.plot3`) file of open edges for BoTs that
 aren't closed volumes. `[replaceable]#bot_name#.plot3` will be
...
|Place `usemtl` statements in the output file. These statements are
 ...
 form
 `[replaceable]##aircode##\_[replaceable]##los##_[replaceable]##material##`,
 where `[replaceable]#aircode#` is the code number for the air
 represented by that region, if it does represent air; otherwise, this
 will be `0`. The `[replaceable]#los#` is the Line Of Sight thickness
 (`0` to `100`) assigned to the region, and `[replaceable]#material#`
...
|[nowrap]`--starting-ident=[replaceable]#number#`
