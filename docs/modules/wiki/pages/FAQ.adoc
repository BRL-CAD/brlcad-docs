...
xref:man:1/mged[mged] as a first step in getting started with BRL-CAD.
...
xref:man:1/pix-png[pix-png] utility can be used to convert from
...
suitable for printing. The xref:man:1/pl-fb[pl-fb] and
xref:man:1/fb-png[fb-png] tools can be used to rasterize a Plot file
to the .png format.
...
xref:man:1/rt[rt] tool is the primary ray-tracer that is used for
shaded view rendering.  The xref:man:1/rtedge[rtedge] and
xref:man:1/rthide[rthide] commands are used for hidden-line renderings
similar to what you'd want for generating drafting
documents. xref:man:1/rtwizard[rtwizard] is a graphical 'wizard' tool
that can be used to generate a variety of hybrid renderings.
