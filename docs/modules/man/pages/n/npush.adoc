...
described once, and then _instances_ of that shape are positioned to
...
_different_ positions and orientations. Matrices associated with
...
The GED command [cmd]*npush* defines logic for "moving" matrix
...
where 3*M1+M2 = 2*M3 (i.e., the matricies on each branch of sph_010's
...
In addition to numerical depth limits, [cmd]*npush* offers two
additional specifiers that are more aware of specific types of objects
in trees.  The [opt]*r* option will push matrices down to a level just
above region references, and the [opt]*s* option will push matrices to
...
A [opt]*s* push, on the other hand, will take the matrices in both
branches deeper:
...
*-v*:: Verbose processing output (primarily used for debugging).

*-f or -x*:: Force creation of new objects if necessary to avoid
conflicts while fully pushing matrices. (A.k.a "xpush" mode).

*-r*:: Halt push operations at the hierarchy level above regions.

*-s*:: Halt push operations at the hierarchy level above solids.

*-d max_depth*:: Halt push operations at the hierarchy level specified
by _max_depth_.
