...
|[nowrap]`--bot-plate-thickness=[var]#thickness#`
...
|Creates a plot/overlay (`.plot3`) file of open edges for BoTs that
 aren't closed volumes. `[var]#bot_name#.plot3` will be
 created in the current directory and will overwrite any existing file
 with the same name.

|`--bot-orientation=[var]#mode#`
...
|Place `usemtl` statements in the output file. These statements are
 ...
 form
 `[var]##aircode##\_[var]##los##_[var]##material##`,
 where `[var]#aircode#` is the code number for the air
 represented by that region, if it does represent air; otherwise, this
 will be `0`. The `[var]#los#` is the Line Of Sight thickness
 (`0` to `100`) assigned to the region, and `[var]#material#`
