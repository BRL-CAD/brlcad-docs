...
[cmd]*rtedge* replaces xref:man:1/lgt.adoc[*lgt*(1)]. In addition to
...
[ui]`rtedge -s1024 -o new.pix -c"set fg=0/255/0 bg=255/0/0" havoc.g havoc`

changes the edge color to be green and the background color to be red. 

[ui]`rtedge -s1024 -o new.pix -c"set rc=1 dr=1" havoc.g havoc`
